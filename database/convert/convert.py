...
def create_one_map_one_table(all_rows, option): 
	...
	if option != "team" and option != "sport_category" and option != "detailed_event":
...
def create_main_events_table(athlete_dict, team_dict, NOC_dict, olympic_games_dict, sport_category_dict, detailed_event_dict, all_rows):
	...
	for row_obj in all_rows:
		...
		this_row = [index, athlete_id, row_obj.age, row_obj.height, row_obj.weight, team_id, NOC_id, oly_game_id, sport_category_id, detailed_event_id]
...
def make_csv_row(this_row):
	...
	for i in range(1, len(this_row)):
		out_csv_row = out_csv_row + "," + str(this_row[i])
...
def main():
	...
	print_table(athlete_table, "athlete_table.csv", athlete_header)
	...
	team_header, sport_categories_header, detailed_events_header = ["ID", "Team"], ["ID", "sport_category"], ["ID", "detailed_event"]
	...
	main_table_header = ["event_ID", "athlete_ID", "age", "height", "weight", "team_ID", "NOC_ID", "oly_game_ID", "sport_category_ID", "detailed_event_ID"]
	main_events_table = create_main_events_table(athlete_dict, team_dict, NOC_dict, olympic_dict, sport_categories_dict, detailed_events_dict, all_rows)
